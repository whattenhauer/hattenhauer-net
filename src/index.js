...
var SECTION_CATEGORIES = ["family", "health", "biblical", "music", "podcasts", "events"];
...
function adsPageHTML(strings, sections, geo, viewerLang, langName, translated) {
  ...
  const sectionsHTML = sections.map((section) => {
    ...
    const itemsHTML = section.items.length > 0 ? section.items.map(
      (item) => '<div class="product-card"><div class="product-tag">' + (item.tag || "") + "</div><h3>" + item.title + "</h3><p>" + item.desc + '</p><div class="product-footer"><span class="product-price">' + (item.price || "") + '</span><a href="' + (item.link || "#") + '" class="product-link" rel="nofollow sponsored noopener" target="_blank">' + strings.viewMore + " \u2192</a></div></div>"
