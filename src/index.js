...
/*
// COMMENTED API INTEGRATION FUNCTIONS — Amazon PA-API, Spotify, PodcastIndex, Eventbrite
  ...
  const sectionsHTML = sections.map(section => {
    ...
    const itemsHTML = section.items.length > 0
      ? section.items.map(item =>
        '<div class="product-card" onmouseup="window.open(\'' + (item.link || '#') + '\', \'_blank\', \'noopener,noreferrer\')" style="cursor:pointer">' +
          '<div class="product-tag">' + (item.tag || '') + '</div>' +
          '<h3>' + item.title + '</h3>' +
          '<p>' + item.desc + '</p>' +
          '<div class="product-footer">' +
            '<span class="product-price">' + (item.price || '') + '</span>' +
            '<a href="' + (item.link || '#') + '" class="product-link" rel="nofollow sponsored noopener" onclick="event.stopPropagation(); return true;">' + strings.viewMore + ' \u2192</a>' +
          '</div>' +
        '</div>'
      ).join('')
