...
async function translateContent(AI, uiStrings, sections, targetLangName) {
  ...
  const prompt = "Translate all text values in this JSON from English to " + targetLangName + ". Keep all keys and structure unchanged. Return ONLY valid JSON, no markdown fences, no explanation.\n\n" + JSON.stringify(payload);
...
function adsPageHTML(strings, sections, geo, viewerLang, langName, translated) {
  ...
  const sectionsHTML = sections.map((section) => {
    ...
    const itemsHTML = section.items.length > 0 ? section.items.map(
      (item) => `<a class="product-card" href="` + (item.link || "#") + `" target="_blank" rel="nofollow sponsored noopener noreferrer"><div class="product-tag">` + (item.tag || "") + "</div><h3>" + item.title + "</h3><p>" + item.desc + '</p><div class="product-footer"><span class="product-price">' + (item.price || "") + '</span><span class="product-link">' + strings.viewMore + " \u2192</span></div></a>"
    ).join("") : '<p class="no-events">' + strings.noEvents + "</p>";
    return '<section class="content-section" id="' + section.id + '"><div class="section-header"><span class="section-icon">' + meta.icon + "</span><div><h2>" + meta.title + "</h2><p>" + meta.desc + '</p></div></div><div class="product-grid">' + itemsHTML + "</div></section>";
  }).join("");
  return '<nav class="ads-nav"><a href="/" class="nav-home">\u2190 ' + strings.backHome + '</a><span class="nav-brand">Hattenhauer</span></nav><header class="ads-hero"><h1>' + strings.pageTitle + "</h1><p>" + strings.pageSubtitle + '</p><div class="geo-badge">\u{1F4CD} ' + strings.detectedLocation + ": <strong>" + locationStr + "</strong>" + (geo.latitude ? " (" + geo.latitude + ", " + geo.longitude + ")" : "") + '</div><div class="lang-badge">\u{1F310} ' + strings.detectedLanguage + ": <strong>" + langName + "</strong>" + (translated ? '<span class="translated-note"> \u2014 ' + strings.autoTranslated + "</span>" : "") + '</div></header><div class="ads-note">\u2139\uFE0F ' + strings.loadingNote + "</div>" + sectionsHTML + '<footer class="ads-footer"><p>&copy; ' + year + ' William Hattenhauer. All rights reserved.</p><p><a href="/">hattenhauer.net</a></p></footer>';
}
__name(adsPageHTML, "adsPageHTML");
function adsHtml(content) {
  const page = '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Hattenhauer \u2014 Discover & Shop</title><meta name="description" content="Context-driven recommendations for family, health, faith, music, podcasts, and local events"><meta name="robots" content="noindex, follow"><style>:root{--teal:#40E0D0;--teal-dark:#20B2AA;--bg-dark:#0a0a0f;--bg-card:rgba(255,255,255,0.03);--text:#e4e4e7;--text-muted:#a1a1aa;--border:rgba(255,255,255,0.08);--tag-bg:rgba(64,224,208,0.12)}*{margin:0;padding:0;box-sizing:border-box}html{scroll-behavior:smooth}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial,sans-serif;background:var(--bg-dark);color:var(--text);line-height:1.6;min-height:100vh}.ads-nav{display:flex;justify-content:space-between;align-items:center;padding:1rem 2rem;border-bottom:1px solid var(--border);position:sticky;top:0;background:rgba(10,10,15,0.85);backdrop-filter:blur(12px);z-index:100}.nav-home{color:var(--teal);text-decoration:none;font-size:0.9rem;transition:opacity 0.2s}.nav-home:hover{opacity:0.7}.nav-brand{font-weight:700;font-size:1.1rem;background:linear-gradient(135deg,var(--teal) 0%,#7dd3fc 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}.ads-hero{text-align:center;padding:3rem 2rem 2rem;max-width:900px;margin:0 auto}.ads-hero h1{font-size:clamp(2rem,5vw,3.5rem);font-weight:800;letter-spacing:-0.02em;margin-bottom:0.75rem;background:linear-gradient(135deg,var(--teal) 0%,#7dd3fc 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}.ads-hero p{color:var(--text-muted);font-size:1.1rem;max-width:560px;margin:0 auto 1.5rem}.geo-badge,.lang-badge{display:inline-block;padding:0.5rem 1.25rem;border-radius:8px;font-size:0.9rem;margin:0.25rem}.geo-badge{background:var(--bg-card);border:1px solid var(--border)}.lang-badge{background:var(--bg-card);border:1px solid var(--border)}.geo-badge strong,.lang-badge strong{color:var(--teal)}.translated-note{color:var(--text-muted);font-size:0.85rem}.ads-note{text-align:center;padding:0.75rem 2rem;color:var(--text-muted);font-size:0.85rem;background:rgba(64,224,208,0.04);border-top:1px solid var(--border);border-bottom:1px solid var(--border)}.content-section{padding:3rem 2rem;max-width:1200px;margin:0 auto}.section-header{display:flex;align-items:flex-start;gap:1rem;margin-bottom:2rem}.section-icon{font-size:2rem;flex-shrink:0}.section-header h2{font-size:1.5rem;font-weight:700;margin-bottom:0.25rem}.section-header h2{color:var(--text)}.section-header p{color:var(--text-muted);font-size:0.95rem}.product-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:1.5rem}.product-card{display:block;text-decoration:none;color:inherit;background:var(--bg-card);border:1px solid var(--border);border-radius:12px;padding:1.5rem;transition:all 0.2s ease;flex-direction:column}.product-card:hover{border-color:rgba(64,224,208,0.2);transform:translateY(-3px)}.product-tag{display:inline-block;padding:0.25rem 0.75rem;border-radius:6px;font-size:0.75rem;font-weight:600;background:var(--tag-bg);color:var(--teal);margin-bottom:0.75rem;align-self:flex-start}.product-card h3{font-size:1.05rem;font-weight:600;margin-bottom:0.5rem;color:var(--text)}.product-card p{color:var(--text-muted);font-size:0.9rem;flex-grow:1;margin-bottom:1rem}.product-footer{display:flex;justify-content:space-between;align-items:center;margin-top:auto}.product-price{font-weight:700;color:var(--teal);font-size:1rem}.product-link{color:var(--text-muted);text-decoration:none;font-size:0.85rem;transition:color 0.2s}.product-card:hover .product-link{color:var(--teal)}.no-events{color:var(--text-muted);font-style:italic;padding:1rem 0}.ads-footer{text-align:center;padding:3rem 2rem;border-top:1px solid var(--border);color:var(--text-muted);font-size:0.9rem}.ads-footer a{color:var(--teal);text-decoration:none}.ads-footer a:hover{text-decoration:underline}@media(max-width:640px){.ads-nav{padding:0.75rem 1rem}.ads-hero{padding:2rem 1rem 1rem}.content-section{padding:2rem 1rem}.section-header{flex-direction:column;gap:0.5rem}}</style></head><body>' + content + "</body></html>";
